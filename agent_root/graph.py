...
# ─────────────────────────────────────────────────────────────────────────────
# SECTION KEY → YAML PROMPT KEY MAPPING
#
# The combined node architecture uses 4 top-level YAML keys, each covering
# 2 granular section keys. When _invoke_section is called with a granular key
# (e.g. "issue_summary"), it must resolve to the correct YAML prompt key
# (e.g. "incident_analysis") before calling build_section_chat_prompt.
#
# Mapping:
#   incident_analysis  → issue_summary, root_cause
#   technical_impact   → impact_analysis, affected_module
#   qa_resolution      → quality_gate_findings, corrective_action
#   prevention_review  → preventive_action, owner_review
# ─────────────────────────────────────────────────────────────────────────────

_SECTION_KEY_TO_PROMPT_KEY: dict[str, str] = {
    "issue_summary":         "incident_analysis",
    "root_cause":            "incident_analysis",
    "impact_analysis":       "technical_impact",
    "affected_module":       "technical_impact",
    "quality_gate_findings": "qa_resolution",
    "corrective_action":     "qa_resolution",
    "preventive_action":     "prevention_review",
    "owner_review":          "prevention_review",
}


def _resolve_prompt_key(section_key: str) -> str:
    """
    Maps a granular section key to the top-level YAML prompt key.

    If the key is already a top-level key (e.g. called directly for a combined
    node), it is returned unchanged. This ensures backward compatibility if any
    caller passes a YAML-level key directly.

    Args:
        section_key: Granular section key (e.g. "root_cause") or YAML key.

    Returns:
        Top-level YAML prompt key (e.g. "incident_analysis").
    """
    return _SECTION_KEY_TO_PROMPT_KEY.get(section_key, section_key)
...
_SANITIZED_SECTION_ERROR = (
    "This section was not available at the time of generation. "
    "Please retry the RCA or contact support."
)
...
def _invoke_section(
    ...
    FIX: section_key is resolved to its parent YAML prompt key via
    _resolve_prompt_key() before calling build_section_chat_prompt. This
    ensures granular keys like "issue_summary" or "root_cause" correctly
    resolve to "incident_analysis" in prompts.yaml, preventing PromptLoadError.

    Provider resolution uses the granular section_key for per-section settings
    lookup, while the prompt is built using the resolved YAML prompt key.
    ...
    Args:
        section_key:     Granular section key (e.g. "root_cause", "issue_summary").
                         Resolved to the YAML prompt key automatically.
    ...
    """
    # Resolve the granular section key to the YAML prompt key
    prompt_key: str = _resolve_prompt_key(section_key)
    ...
    #
    # Provider/model resolution uses the YAML prompt key (combined node key)
    # so that generation_config overrides like "incident_analysis" apply
    # correctly to both "issue_summary" and "root_cause" sub-sections.
    gen_cfg: Optional[RCAGenerationConfig] = rca_input.generation_config

    primary_provider: str = (
        (gen_cfg.get_provider(prompt_key) if gen_cfg else None)
        or getattr(settings, f"{prompt_key}_provider", None)
        or settings.primary_provider
        or "openai"
    )
    primary_model: str = (
        (gen_cfg.get_model(prompt_key) if gen_cfg else None)
        or getattr(settings, f"{prompt_key}_model", None)
    ...
    # Build prompt using the resolved YAML prompt key (not the granular key)
    prompt = build_section_chat_prompt(
        section_key=prompt_key,
        rca_input=rca_input,
...
def supervisor_validate(
    ...
    is_sanitized_error = (
        ...
        or "was not available at the time of generation" in lowered
    )
...
#
# Each granular section key is resolved to its YAML prompt key via
# _resolve_prompt_key() inside _invoke_section, so prompts.yaml only needs
# the 4 combined node keys.
#
...
def generate_incident_analysis(state: RCAGraphState) -> dict[str, Any]:
    ...
    Both sub-sections resolve to the "incident_analysis" YAML prompt key.
    """
    ...
    # ── Step 1: issue_summary → resolves to "incident_analysis" in YAML ──────
    issue_summary_text = _invoke_section_with_retry(
    ...
    # ── Step 2: root_cause → resolves to "incident_analysis" in YAML ─────────
    root_cause_context = dict(base_context)
    root_cause_context["section_issue_summary"] = issue_summary_text

    root_cause_text = _invoke_section_with_retry(
...
def generate_technical_impact(state: RCAGraphState) -> dict[str, Any]:
    ...
    Both sub-sections resolve to the "technical_impact" YAML prompt key.
    """
    issue_id: str   = state.get("issue_id", "unknown")  # type: ignore[assignment]
    rca_input       = state["rca_input"]
    quality_summary = state.get("quality_summary", "")
    ...
    # ── Step 1: impact_analysis → resolves to "technical_impact" in YAML ─────
    impact_analysis_text = _invoke_section_with_retry(
...
def generate_qa_resolution(state: RCAGraphState) -> dict[str, Any]:
    ...
    Both sub-sections resolve to the "qa_resolution" YAML prompt key.
    """
    issue_id: str   = state.get("issue_id", "unknown")  # type: ignore[assignment]
    rca_input       = state["rca_input"]
    quality_summary = state.get("quality_summary", "")
    ...
    # ── Step 1: quality_gate_findings → resolves to "qa_resolution" in YAML ──
    qg_findings_text = _invoke_section_with_retry(
    ...
    # ── Step 2: corrective_action → resolves to "qa_resolution" in YAML ──────
    ...
    corrective_action_text = _invoke_section_with_retry(
...
def generate_prevention_review(state: RCAGraphState) -> dict[str, Any]:
    ...
    Both sub-sections resolve to the "prevention_review" YAML prompt key.
    """
    issue_id: str   = state.get("issue_id", "unknown")  # type: ignore[assignment]
    rca_input       = state["rca_input"]
    quality_summary = state.get("quality_summary", "")
    ...
    # ── Step 1: preventive_action → resolves to "prevention_review" in YAML ──
    preventive_context = dict(base_context)
    if corrective_action:
        preventive_context["section_corrective_action"] = corrective_action

    preventive_action_text = _invoke_section_with_retry(
...
def _invoke_section_with_retry(
    ...
    Args:
        section_key:            Granular section key (e.g. "root_cause").
                                Resolved to the YAML prompt key inside _invoke_section.
